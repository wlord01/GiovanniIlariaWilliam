...
def test_multiple_trials(model_type, trials, number_of_simulations):
    ...
    for each in the current scenario and returning data on goal
    accomplishing and number of required steps for goal based planning,
    and data on reward for utility reasoning experiments. Set
    graphics_on to False in phase_2.py for much faster runtime. Saves
    data to files.
    """
    return


def check_completion(multiple_trials_data):
    """Check which trials accomplished the goal

    Keyword arguments:
    - multiple_trials_data -- data as generated by function
      test_multiple_trials()

    Checks which of the trials completed the goal in more than half
    of the simulation runs. Returns data array of successful trials.
    """
    return


def check_completion_ratio(successful_trials_data):
